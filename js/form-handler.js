...
document.addEventListener("DOMContentLoaded", function () {
  ...
  document.querySelectorAll(".dropdown-item, button").forEach((element) => {
    // ปุ่มดาวน์โหลด
    if (
      element.innerHTML.includes("ดาวน์โหลด") ||
      element.id === "download-chart"
    ) {
    ...
    if (
      element.innerHTML.includes("รีเฟรช") &&
      element.id !== "refresh-btn" &&
      element.id !== "refresh-chart"
    ) {
      element.addEventListener("click", function (e) {
        e.preventDefault();
    ...
    if (
      element.innerHTML.includes("รายวัน") ||
      element.innerHTML.includes("รายสัปดาห์")
    ) {
      element.addEventListener("click", function (e) {
        e.preventDefault();
    ...
    if (
      element.innerHTML.includes("กราฟแท่ง") ||
      element.innerHTML.includes("กราฟวงกลม")
    ) {
      element.addEventListener("click", function (e) {
        e.preventDefault();
        showToast("กำลังพัฒนาฟีเจอร์นี้", "info");
  ...
  document
    ...
    .addEventListener("submit", function (e) {
      ...
      if (sourceName === "") {
        showToast("กรุณาเลือกกล้อง", "warning");
        return;
      }
